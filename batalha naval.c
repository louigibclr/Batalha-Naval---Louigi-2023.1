...
int isValido(char tabuleiro[TAMANHO][TAMANHO], int tam, int x, int y) {
  for (int i = 0; i < tam; i++){
    if (!isEqual(tabuleiro[x][y+i], AGUA)) 
    {
      return 0;
    } 
    else if (!isEqual(tabuleiro[x+i][y], AGUA)) 
    {
      return 0;
    }
  }
...
void posicionarSubmarino(char tabuleiro[TAMANHO][TAMANHO])
{
    ...
    for (int i = 0; i < quantSUBMARINO; i++)
    {
        ...
        do
        {
            x = rand() % TAMANHO;
            y = rand() % TAMANHO;
        } while (!isValido(tabuleiro, tamSUBMARINO, x, y));
...
void posicionarNavioHumano(int tam_navio, char tabuleiro[TAMANHO][TAMANHO], int quantidade){
    ...
    linhaNavioJogador = toupper(linhaNavioJogador);
        ...
        if(escolha == 'h') {
            xlimit = TAMANHO;
            ylimit = TAMANHO - tam_navio + 1;
            if (isValidoHorizontal(tabuleiro, tam_navio, colunaNavioJogador, linhaNavioJogador)) {
            for (int i = 0; i < tam_navio; i++) {
                if (linhaNavioJogador - 'A' > xlimit || (colunaNavioJogador - 1) + i > ylimit ) {
                    printf("Essa posicao ultrapassa o limite do tabuleiro. Escolha outra.\n");
                } 
                else 
                {
            ...
            xlimit = TAMANHO - tam_navio + 1;
            ylimit = TAMANHO;
            if (isValidoVertical(tabuleiro, tam_navio, colunaNavioJogador, linhaNavioJogador)) {
            for (int i = 0; i < tam_navio; i++) {
                if ((linhaNavioJogador - 'A') + i > xlimit || colunaNavioJogador - 1 > ylimit ) {
                    printf("Essa posicao ultrapassa o limite do tabuleiro. Escolha outra.\n");
                } 
                else 
                {
